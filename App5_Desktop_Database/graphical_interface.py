...
def view_all_data():
    ...
    for row in backend_data.view():
        data_view.insert(END, row)


def search_item():
    data_view.delete(0, END)
    for row in backend_data.search(title_text.get(), author_text.get(), year_text.get(), isbn_text.get()):
        data_view.insert(END, row)


def add_item():
    backend_data.insert(title_text.get(), author_text.get(), year_text.get(), backend_data.isbn_generator())
    data_view.delete(0, END)
    data_view.insert(END, (title_text.get(), author_text.get(), year_text.get(),
                           backend_data.isbn_generator()))


# Uses helper function for selecting the data item in the listbox
def delete_item():
    backend_data.delete(selected[0])


def update_item():
    backend_data.update(selected[0], title_text.get(), author_text.get(), year_text.get(), isbn_text.get())
